...
def listInstance(ec2):
    ...
    for instance in ec2_list:
        print("[id]  " + instance.id + "  [AMI]  " + instance.image_id + "  [type]  " + instance.instance_type +
              "  [state]  " + instance.state["Name"] + "  [mornitoring state]  " + instance.monitoring['State'] + '\n')
...
def availableZone(ec2):
    ...
    for instance in instances:
        print("[id]  " + instance['ZoneId'] + "  [region]  " +
              instance['RegionName'] + "  [zone]  " + instance['ZoneName'])
        cnt += 1
...
def availableRegion(ec2):
    ...
    for instance in instances:
        print("[region]  " + instance['RegionName'] +
...
def createInstance(ec2):
    ...
    print("Successfully started EC2 instance " +
          instances[0].instance_id + " based on AMI " + Input_id)
...
def listImage(ec2):
    ...
    for image in image_list:
        print("[ImageID]  " + image.id + "  [Name]  " +
              image.name + "  [Owner]  " + image.owner_id)
...
def createImage(ec2):
    ...
    try:
        ...
        print("Successfully create image of " + Input_id)
...
def deleteImage(ec2):
    ...
    try:
        ...
        print("Successfully delete image of " + Input_id)
    except ClientError as e:
        print(e)


def allocateAddress(ec2):
    ...
    try:
        ...
        print("Successfully allocate ip : " + allocation['PublicIp'])
    except ClientError as e:
        print(e)
...
def listAddress(ec2):
    ...
    for instance in response['Addresses']:
        print("[Public Ip]   " + instance['PublicIp'] + "  [Allocation Id]  " +
              instance['AllocationId'] + "  [NetworkBorderGroup]  " + instance['NetworkBorderGroup'])


def associateAddress(ec2):
    Input_id = input("Enter instance id : ")
    Input_ip = input("Enter elastic Id : ")
    ...
    try:
        response = client.associate_address(
            AllocationId=Input_ip,
            InstanceId=Input_id
        )
        print("Successfully associate ip\n")
    except ClientError as e:
        print(e)
...
def main():
    ...
    while True:
        ...
        print("  9. condor_status                10. create image          \n")
        print("  11. delete image                12. allocate elastic IP   \n")
        print("  13. list elastic ip             14. associate elastic ip  \n")
        print("                                  99. quit                  \n")
        print("------------------------------------------------------------")
